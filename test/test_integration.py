...
class TestIntegration(unittest.TestCase):
    ...
    def test_integration(self):
        ...
        invoice.create_external_charges(account_id, [body], 'test', random_api_key, random_api_secret, auto_commit=True)

        # Verify account balance
        account = killbill.api.AccountApi()
        account = account.get_account(account_id, random_api_key, random_api_secret, account_with_balance=True)
        self.assertEqual(50.0, account.account_balance)

        # Pay all unpaid invoices
        account = killbill.api.AccountApi()
        account.pay_all_invoices(account_id, 'test', random_api_key, random_api_secret, external_payment=True)
        ...
        payment.refund_payment(payment_id, body, 'test', random_api_key, random_api_secret)
    ...
    def test_pagination_and_search(self):
        ...
        for x in range(0, 10):
            ...
            invoice = killbill.api.InvoiceApi()
            body = InvoiceItem(account_id=account_id, amount=50, currency='USD', description='My charge')
            invoice.create_external_charges(account_id, [body], 'test', random_api_key, random_api_secret,
                                            auto_commit=True)
        ...
        invoice = killbill.api.InvoiceApi()
        invoices = invoice.get_invoices(random_api_key, random_api_secret)
        self.assertEqual(10, len(invoices))

        # Pagination list payments
        payment = killbill.api.PaymentApi()
        payments = payment.get_payments(random_api_key, random_api_secret)
        self.assertEqual(10, len(payments))

        # Search accounts
        account = killbill.api.AccountApi()
        accounts = account.search_accounts('John-1', random_api_key, random_api_secret)
        self.assertEqual(1, len(accounts))

        # Search payments
        payment = killbill.api.PaymentApi()
        payments = payment.search_payments('SUCCESS', random_api_key, random_api_secret)
        self.assertEqual(10, len(payments))

        # Search invoices
        invoice = killbill.api.InvoiceApi()
        invoices = invoice.search_invoices('USD', random_api_key, random_api_secret)
        self.assertEqual(10, len(invoices))
